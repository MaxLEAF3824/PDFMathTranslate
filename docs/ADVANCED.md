...
|**GitHub Copilot**|`github-copilot`| `GITHUB_COPILOT_TOKEN`, `GITHUB_COPILOT_MODEL` | `[Your OAuth Token]`, `gpt-4.1` | Reuse your paid GitHub Copilot ("coding plan") subscription as the translation backend through the official Copilot SDK. `GITHUB_COPILOT_TOKEN` is a GitHub OAuth token with Copilot access (usually starts with `gho_`/`ghu_`); if omitted, it is read from the local credential file created by official Copilot plugins (e.g. `~/.config/github-copilot/apps.json`). See [GitHub Copilot](https://github.com/features/copilot). |
